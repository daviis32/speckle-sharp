...
MemoryMapped::MemoryMapped(const std::string& mapName, size_t mappedBytes, CacheHint hint)
    : _filesize(0),
    _hint(hint),
...
/// open file
bool MemoryMapped::open(const std::string& mapName, size_t mappedBytes, CacheHint hint)
{
    // already open ?
    if (isValid())
        return false;
    ...
    GetTempFileName(lpTempPathBuffer, TEXT("ac_connector"), 0, szTempFileName);
    _file = ::CreateFile(szTempFileName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, winHint, NULL);
    ...
    char* buff = (char*)malloc(mappedBytes);
    DWORD bytesWritten = 0;
    WriteFile(_file, buff, (DWORD)mappedBytes, &bytesWritten, NULL);
    ...
    mapName;
    TCHAR szMapName[] = TEXT("ac_connector");
    _mappedFile = ::CreateFileMapping(_file, NULL, PAGE_READWRITE, 0, (DWORD)mappedBytes, szMapName);
    if (!_mappedFile) {
        auto err = GetLastError();
        ::printf("Error: %d\n", err);
        return false;
...
void MemoryMapped::setData(const char* data, size_t len)
{
    unsigned char s1 = (unsigned char) (len / 256);
    unsigned char s2 = (unsigned char)(len - s1 * 256);
    ((char*)_mappedView)[0] = s1;
    ((char*)_mappedView)[1] = s2;
    memcpy((char*)_mappedView+2, data, len);
    ((char*)_mappedView)[len+2] = 0;
}
