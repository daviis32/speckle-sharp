...
public:
    ...
    MemoryMapped(const std::string& mapName, size_t mappedBytes = WholeFile, CacheHint hint = Normal);
    /// close file (see close() )
    ~MemoryMapped();

    /// open file, mappedBytes = 0 maps the whole file
    bool open(const std::string& mapName, size_t mappedBytes = WholeFile, CacheHint hint = Normal);
    /// close file
    void close();
