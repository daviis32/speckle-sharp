...
namespace AddOnCommands {
	...
	static void StoreModelOfElements(const GS::Array<std::string>& applicationIds, rapidjson::Document& output)
	{
		...
		rapidjson::Value models(kArrayType);
		...
		for (const auto& applicationId : applicationIds) {
			rapidjson::Value modelElement(kObjectType);

			modelElement[ApplicationIdFieldName] = Value().SetString(applicationId, output.GetAllocator());

			rapidjson::Value model(kObjectType);
			//CalculateModelOfElement(modelViewer, applicationId, model);
			modelElement[Model::ModelFieldName] = model;


			models.PushBack(modelElement, output.GetAllocator());
			//modelInserter(GS::ObjectState{ ApplicationIdFieldName, applicationId, Model::ModelFieldName, CalculateModelOfElement(modelViewer, applicationId) });
		}

		output[ModelsFieldName] = models;
...
//		return StoreModelOfElements(ids.Transform<API_Guid>([](const GS::UniString& idStr) { return APIGuidFromString(idStr.ToCStr()); }));
	...
	bool Cmd_GetModelForElements::Execute(const rapidjson::Value& parameters, rapidjson::Document& output)
	{
		using namespace rapidjson;
		...
		if (!parameters.IsNull()) {
			if (parameters.HasMember("applicationIds")) {
				...
				GS::Array<std::string> ids;
				ret = true;
				for (rapidjson::SizeType i = 0; i < applicationIds.Size(); i++) {
					ids.Push(applicationIds[i].GetString());
				}

				StoreModelOfElements(ids, output);
			}
		}
	...
	std::string Cmd_GetModelForElements::GetName() const
	{
		return GetModelForElementsCommandName;
	}
